...
import arx.engine.control.event.{MouseButton, MouseMoveEvent, MouseReleaseEvent, UIEvent}
...
class DynamicWidgetData extends TWidgetAuxData {
	var dynWidgetFunctions : DynamicWidgetFunctions = new DynamicWidgetFunctions {
		...
		override def createChildrenFromData(dynWidget: Widget, data: Any): Seq[Widget] = Vector(dynWidget.createChild(SimpleWidget).widget)
...
trait DynamicWidgetFunctions {
	...
	def createChildrenFromData(dynWidget: Widget, data : Any) : Seq[Widget]
...
//								Noto.warn(s"list binding is to non sequence, that won't work: $other")
				...
				if (dynWidget[ListWidgetData].selectable) {
					child.onEvent {
				...
				child.onEvent {
					case MouseMoveEvent(_,_,_) =>
						dynWidget.handleEvent(ListItemMousedOver(dynWidget.effectiveIdentifier, index, child.resolveBinding(bindingTo)))
				}


				val separator = dynWidget[ListWidgetData].separator match {
					case Some(separatorArch) if index != 0 => Some(dynWidget.createChild(separatorArch))
					case _ => None
				}

				separator.toVector ++ Vector(child)
			case _ => ???
		}
	}
...
class ListWidgetData extends TWidgetAuxData {
	...
	/**
	 * Determines whether or not the individual items can be selected
	 */
...
case class ListItemMousedOver(listId : String, index : Int, data : Option[Any]) extends UIEvent