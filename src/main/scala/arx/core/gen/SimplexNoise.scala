 ...
 * @author Aleksey Nikiforov (lex)
  ...
  * ++++++++++ Mine own
  *
  * Adapted to fit into personal framework, minor syntax fixes, added
  * single precision floating point implementations of the various functions
  * @author Samuel Bock (nvt)
 */
class SimplexNoise(val seed: Long) extends Serializable {

  import SimplexNoise._

  
  private[this] final val perm: Array[Byte] = {
  ...
  private final def ifloor(x: Double) :Long = {
	...
	private final def ifloorf(x: Float) :Long = {
		val i = x.toLong
		if (x > 0 || x == i) i else i - 1
	}
  ...
  final def noise(x: Double) :Double = {
	...
	/** Computes 1D simplex noise.
	   ...
	   * @return simplex noise value for the specified coordinate.
	   */
	  final def noise(x: Float) :Float = {
	    // The implementation is restricted to arguments in range [-2E8, +2E8] when using Ints.
	    val pix = ifloorf(x*F1f)
  ...
  final def noise(x: Double, y: Double) :Double = {
	...
	/** Computes 2D simplex noise. Somewhat slower but much better looking
	   ...
	   * @return simplex noise value for the specified coordinates.
	   */
	  final def noise(x: Float, y: Float) :Float = {
	    // Skew the (x,y) space to determine which cell of 2 simplices we're in.
	    val s = (x + y) * F2f // Hairy factor for 2D skewing.

	    // The implementation is restricted to arguments in range [-2E8, +2E8] when using Ints.
	    val pix = ifloorf(x + s)
	    val piy = ifloorf(y + s)
	    val t = (pix + piy) * G2f // Hairy factor for unskewing.
	    ...
	    // Sum up and scale the result to cover the range [-1,1].
	    70.14805770653953038786f * (n0 + n1 + n2)
	  }
  ...
  final def noise(x: Double, y: Double, z:Double) :Double = {
	...
	/** Computes 3D simplex noise. Comparable in speed to classic (Perlin) noise,
	   ...
	   * @return simplex noise value for the specified coordinates.
	   */
	  final def noise(x: Float, y: Float, z:Float) :Float = {
	    // Skew the (x,y,z) space to determine which cell of 6 simplices we're in.
	    val s = (x + y + z) * F3f // Factor for 3D skewing.

	    // The implementation is restricted to arguments in range [-2E8, +2E8] when using Ints.
	    val pix = ifloorf(x + s)
	    val piy = ifloorf(y + s)
	    val piz = ifloorf(z + s)
	    val t = (pix + piy + piz) * G3f
	    ...
	    // Noise contribution from simplex origin.
	    val t0 = 0.5f - p0x*p0x - p0y*p0y - p0z*p0z
	    val n0 =
	      if (t0 < 0.0f) 0.0f
	      else {
	        val pz = perm(iz)
	    ...
	    val p1y = p0y - o1y + G3f
	    val p1z = p0z - o1z + G3f
	    val t1 = 0.5f - p1x*p1x - p1y*p1y - p1z*p1z
	    val n1 : Float =
	      if (t1 < 0.0f) 0.0f
	      else {
	        val pz = perm((iz + o1z) & 0xFF)
	    ...
	    // Noise contribution from third corner.
	    val p2x = p0x - o2x + G32f
	    val p2y = p0y - o2y + G32f
	    val p2z = p0z - o2z + G32f
	    val t2 = 0.5f - p2x*p2x - p2y*p2y - p2z*p2z
	    val n2 =
	      if (t2 < 0.0f) 0.0f
	      else {
	        val pz = perm((iz + o2z) & 0xFF)
	    ...
	    val p3y = p0y - G33f
	    val p3z = p0z - G33f
	    val t3 = 0.5f - p3x*p3x - p3y*p3y - p3z*p3z
	    val n3 =
	      if(t3 < 0.0f) 0.0f
	      else {
	        val pz = perm((iz + 1) & 0xFF)
   ...
   * @return simplex noise value for the specified coordinates.
   */
  final def noise(x: Float, y: Float, z:Float, w:Float) :Float = {
    // Skew the (x,y,z,w) space to determine which cell of 24 simplices we're in.
    val s = (x + y + z + w) * F4f // Factor for 4D skewing.
    ...
    // Noise contribution from simplex origin.
    val t0 = 0.5f - p0x*p0x - p0y*p0y - p0z*p0z - p0w*p0w
    val n0 =
      if (t0 < 0.0f) 0.0f
      else {
        val pw = perm(iw)
    ...
    val p1z = p0z - o1z + G4f
    val p1w = p0w - o1w + G4f
    val t1 = 0.5f - p1x*p1x - p1y*p1y - p1z*p1z - p1w*p1w
    val n1 =
      if (t1 < 0.0f) 0.0f
      else {
        val pw = perm((iw + o1w) & 0xFF)
    ...
    // Noise contribution from third corner.
    val p2x = p0x - o2x + G42f
    val p2y = p0y - o2y + G42f
    val p2z = p0z - o2z + G42f
    val p2w = p0w - o2w + G42f
    val t2 = 0.5f - p2x*p2x - p2y*p2y - p2z*p2z - p2w*p2w
    val n2 =
      if (t2 < 0.0f) 0.0f
      else {
        val pw = perm((iw + o2w) & 0xFF)
    ...
    // Noise contribution from fourth corner.
    val p3x = p0x - o3x + G43f
    val p3y = p0y - o3y + G43f
    val p3z = p0z - o3z + G43f
    val p3w = p0w - o3w + G43f
    val t3 = 0.5f - p3x*p3x - p3y*p3y - p3z*p3z - p3w*p3w
    val n3 =
      if (t3 < 0.0f) 0.0f
      else {
        val pw = perm((iw + o3w) & 0xFF)
    ...
    val p4z = p0z - G44f
    val p4w = p0w - G44f
    val t4 = 0.5f - p4x*p4x - p4y*p4y - p4z*p4z - p4w*p4w
    val n4 =
      if(t4 < 0.0f) 0.0f
      else {
        val pw = perm((iw + 1) & 0xFF)
    ...
    // Sum up and scale the result to cover the range [-1,1].
    62.777711789695920f * (n0 + n1 + n2 + n3 + n4)
  }
}
...
object SimplexNoise extends SimplexNoise(0) {
  ...
  final val offset3 = 22.3*0.7071067811865475244
	...
	def ByteArray ( a : Int , b : Int , c : Int ) = Array(a.toByte,b.toByte,c.toByte)
	def ByteArray ( a : Int , b : Int , c : Int , d : Int ) = Array(a.toByte,b.toByte,c.toByte,d.toByte)
	
  private final val grad3: Array[Array[Byte]] = Array(
    ByteArray(0,1,1), ByteArray(0,1,-1), ByteArray(0,-1,1), ByteArray(0,-1,-1),
    ByteArray(1,0,1), ByteArray(1,0,-1), ByteArray(-1,0,1), ByteArray(-1,0,-1),
    ByteArray(1,1,0), ByteArray(1,-1,0), ByteArray(-1,1,0), ByteArray(-1,-1,0),
    ByteArray(1,0,-1), ByteArray(-1,0,-1), ByteArray(0,-1,1), ByteArray(0,1,1)
  )

  private final val grad4: Array[Array[Byte]] = Array(
    ByteArray(0,1,1,1), ByteArray(0,1,1,-1), ByteArray(0,1,-1,1), ByteArray(0,1,-1,-1),
    ByteArray(0,-1,1,1), ByteArray(0,-1,1,-1), ByteArray(0,-1,-1,1), ByteArray(0,-1,-1,-1),
    ByteArray(1,0,1,1), ByteArray(1,0,1,-1), ByteArray(1,0,-1,1), ByteArray(1,0,-1,-1),
    ByteArray(-1,0,1,1), ByteArray(-1,0,1,-1), ByteArray(-1,0,-1,1), ByteArray(-1,0,-1,-1),
    ByteArray(1,1,0,1), ByteArray(1,1,0,-1), ByteArray(1,-1,0,1), ByteArray(1,-1,0,-1),
    ByteArray(-1,1,0,1), ByteArray(-1,1,0,-1), ByteArray(-1,-1,0,1), ByteArray(-1,-1,0,-1),
    ByteArray(1,1,1,0), ByteArray(1,1,-1,0), ByteArray(1,-1,1,0), ByteArray(1,-1,-1,0),
    ByteArray(-1,1,1,0), ByteArray(-1,1,-1,0), ByteArray(-1,-1,1,0), ByteArray(-1,-1,-1,0)
  )
  ...
  private final val G1 = 0.7071067811865475244 //1 / Math.sqrt(2.0)
	...
	private final val F1f = 1.4142135623730950488f //Math.sqrt(2.0)
	private final val G1f = 0.7071067811865475244f //1 / Math.sqrt(2.0)
  ...
  private final val G22 = 0.57735026918962576451 //1 - 2.0 * (3.0 - Math.sqrt(3.0)) / 6.0
	...
	private final val F2f = 0.36602540378443864676f //(Math.sqrt(3.0) - 1.0) / 2.0
	private final val G2f = 0.21132486540518711775f //(3.0 - Math.sqrt(3.0)) / 6.0
	private final val G22f = 0.57735026918962576451f //1 - 2.0 * (3.0 - Math.sqrt(3.0)) / 6.0
  ...
  private final val G33 = 1 - 3 / 6.0
	...
	private final val F3f = 1 / 3.0f
	private final val G3f = 1 / 6.0f
	private final val G32f = 2 / 6.0f
	private final val G33f = 1 - 3 / 6.0f
  ...
  private final val G44 = 0.44721359549995793928 //1 - 4.0 * ((5.0 - Math.sqrt(5.0)) / 20.0)
	...
	private final val F4f = 0.3090169943749474241f //(Math.sqrt(5.0) - 1.0) / 4.0
	private final val G4f = 0.13819660112501051518f //(5.0 - Math.sqrt(5.0)) / 20.0
	private final val G42f = 0.27639320225002103036f //2.0 * ((5.0 - Math.sqrt(5.0)) / 20.0)
	private final val G43f = 0.41458980337503154554f //3.0 * ((5.0 - Math.sqrt(5.0)) / 20.0)
	private final val G44f = 0.44721359549995793928f //1 - 4.0 * ((5.0 - Math.sqrt(5.0)) / 20.0)
}
