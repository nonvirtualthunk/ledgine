...
import java.util.concurrent.{ConcurrentHashMap, ConcurrentSkipListMap, ConcurrentMap => JConcurrentMap}
...
class AtomicMap[A,B](u : => JConcurrentMap[A,Any]) extends concurrent.Map[A,B] {
  lazy val under = u
