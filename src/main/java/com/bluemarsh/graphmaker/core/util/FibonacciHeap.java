...
public class FibonacciHeap<T> {
    ...
    private int createdNodeIndex = 0;
    private ArrayList<Node<T>> createdNodes = new ArrayList<Node<T>>();
    ...
    protected Node<T> borrowNode( T x , float key ) {
        if ( createdNodeIndex >= createdNodes.size() ) {
            Node<T> ret = new Node<T>(x,key);
            createdNodes.add(ret);
            createdNodeIndex += 1;
            return ret;
        } else {
           Node<T> node = createdNodes.get(createdNodeIndex);
            createdNodeIndex += 1;
           node.data = x;
           node.key = key;
           node.left = node;
